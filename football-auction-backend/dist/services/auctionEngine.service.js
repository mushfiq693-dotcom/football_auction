...
exports.AuctionEngineService = void 0;
...
class AuctionEngineService {
     ...
     */
    static calculateDynamicIncrements(currentBid, totalBudget = 100000, categoryBasePrice = 1000) {
    ...
    static async placeBid(dto) {
        return await database_1.prisma.$transaction(async (tx) => {
            ...
            if (!session || session.status !== client_1.AuctionStatus.ACTIVE) {
                throw new errorHandler_middleware_1.AppError(400, 'Auction session is not currently active for bidding');
            }
            ...
            if (!wallet) {
                throw new errorHandler_middleware_1.AppError(404, 'Team wallet not found');
            }
            if (wallet.playersBoughtCount >= wallet.maxPlayerLimit) {
                throw new errorHandler_middleware_1.AppError(400, `Team has already reached maximum squad limit of ${wallet.maxPlayerLimit} players`);
            }
            if (wallet.currentBalance < dto.amount) {
                throw new errorHandler_middleware_1.AppError(400, `Insufficient budget balance. Available: $${wallet.currentBalance}, Bid: $${dto.amount}`);
            }
            ...
            if (remainingBalanceAfterBid < requiredReserve) {
                throw new errorHandler_middleware_1.AppError(400, `Budget Guardrail Violation: Placing a bid of $${dto.amount} leaves $${remainingBalanceAfterBid}. You must retain at least $${requiredReserve} in reserve to buy your remaining ${remainingSquadNeeded} required squad slot(s) at lowest base price $${lowestBasePriceInSystem}.`);
            }
            // 4. Handle BLIND AUCTION vs NORMAL AUCTION
            if (session.auctionType === client_1.AuctionType.BLIND) {
                if (dto.amount < basePrice) {
                    throw new errorHandler_middleware_1.AppError(400, `Sealed bid amount must be at least the base price of $${basePrice}`);
                }
            ...
            if (session.currentBid === 0) {
                if (dto.amount < basePrice) {
                    throw new errorHandler_middleware_1.AppError(400, `Opening bid must be at least the base price of $${basePrice}`);
                }
            }
            else {
                if (dto.amount <= session.currentBid) {
                    throw new errorHandler_middleware_1.AppError(400, `Bid amount ($${dto.amount}) must be strictly higher than current top bid ($${session.currentBid})`);
                }
                if (dto.amount < session.currentBid + dynamicMath.minRaise) {
                    throw new errorHandler_middleware_1.AppError(400, `Bid must be higher by at least the minimum raise of $${dynamicMath.minRaise} (Minimum next bid: $${session.currentBid + dynamicMath.minRaise})`);
                }
            }
            // Record valid bid
            const bid = await tx.auctionBid.create({
     ...
     */
    static async finalizeAuction(auctionSessionId) {
        return await database_1.prisma.$transaction(async (tx) => {
            const session = await tx.auctionSession.findUnique({
            ...
            if (!session)
                throw new errorHandler_middleware_1.AppError(404, 'Auction session not found');
            ...
            if (session.auctionType === client_1.AuctionType.BLIND && session.bids.length > 0) {
            ...
            if (!winningTeamId || winningAmount === 0) {
                const updatedSession = await tx.auctionSession.update({
                    where: { id: auctionSessionId },
                    data: { status: client_1.AuctionStatus.UNSOLD },
                });
     ...
     */
    static async toggleAuctionStatus(auctionSessionId, status) {
        return await database_1.prisma.auctionSession.update({
            where: { id: auctionSessionId },
            data: { status },
            include: {
    ...
    static async rollbackAuction(auctionSessionId) {
        return await database_1.prisma.$transaction(async (tx) => {
...
exports.AuctionEngineService = AuctionEngineService;
