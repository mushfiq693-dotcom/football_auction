...
export class NukeService {
   ...
   */
  static async level1TournamentWipe(adminUserId: string) {
    return await prisma.$transaction(async (tx) => {
      ...
      if (globalState) {
        await tx.globalState.update({
          ...
          data: { activePhase: Phase.LIVE_AUCTION, updatedBy: adminUserId },
        });
      }
      ...
      return {
        ...
        activePhase: Phase.LIVE_AUCTION,
      };
    });
  }
   ...
   */
  static async level2RosterWipe(adminUserId: string) {
    ...
    if (publicIds.length > 0) {
      await CloudinaryService.deleteAssets(publicIds);
    }
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      await tx.user.deleteMany({
        where: {
          role: {
            in: [Role.TEAM_OWNER, Role.PLAYER, Role.PUBLIC_GUEST],
          },
        },
      });
      ...
      if (globalState) {
        await tx.globalState.update({
          where: { id: globalState.id },
          data: { activePhase: Phase.SETUP, updatedBy: adminUserId },
        });
      }
   ...
   * Reverts system to Phase 1 (SETUP).
   */
  static async level3FactoryReset(adminUserId: string) {
